...
interface LinkRepository {
    ...
    fun createLink(name: String?): Long

    fun getLinks(caller: String?): List<Link>

    fun addToLink(linkId: Long, ids: LongArray): Int

    fun getSongsInLink(linkId: Long, caller: String?): List<Song>

    fun removeFromLink(linkId: Long, id: Long)

    fun deleteLink(linkId: Long): Int
}

class RealLinkRepository(
        private val contentResolver: ContentResolver
) : LinkRepository {

    override fun createLink(name: String?): Long {
    ...
    override fun getLinks(caller: String?): List<Link> {
        MediaID.currentCaller = caller
        return makePlaylistCursor().mapList(true) {
            val id: Long = value(_ID)
            val songCount = getSongCountForPlaylist(id)
            Link.fromCursor(this, songCount)
        }.filter { it.name.isNotEmpty() }
    }

    override fun addToLink(linkId: Long, ids: LongArray): Int {
        val projection = arrayOf("max($PLAYLIST_PLAY_ORDER)")
        val uri = MediaStore.Audio.Playlists.Members.getContentUri("external", linkId)
    ...
    override fun getSongsInLink(linkId: Long, caller: String?): List<Song> {
        MediaID.currentCaller = caller
        val playlistCount = countPlaylist(linkId)

        makeLinkSongCursor(linkId)?.use {
            ...
            if (runCleanup) {
                cleanupPlaylist(linkId, it, true)
            }
        }

        return makeLinkSongCursor(linkId)
                .mapList(true, Song.Companion::fromPlaylistMembersCursor)
    }

    override fun removeFromLink(linkId: Long, id: Long) {
        val uri = MediaStore.Audio.Playlists.Members.getContentUri("external", linkId)
    ...
    override fun deleteLink(linkId: Long): Int {
    ...
    private fun makeLinkSongCursor(linkID: Long?): Cursor? {
        ...
        return contentResolver.query(
                MediaStore.Audio.Playlists.Members.getContentUri("external", linkID!!),
                projection,
                selection.toString(),
                null,
                MediaStore.Audio.Playlists.Members.DEFAULT_SORT_ORDER
