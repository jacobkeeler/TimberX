...
import com.naman14.timberx.playback.TimberMusicService.Companion.TYPE_LINK
...
data class Link(
        ...
        MediaDescriptionCompat.Builder()
                .setMediaId(MediaID(TYPE_LINK.toString(), id.toString()).asString())
                .setTitle(name)
                .setSubtitle("$songCount songs")
                .build(), FLAG_BROWSABLE) {
    companion object {
        fun fromCursor(cursor: Cursor, songCount: Int): Link {
            return Link(
                    id = cursor.value(_ID),
